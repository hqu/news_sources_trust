"""Write rankings_standalone.html — rankings.html with data.json embedded, no server needed.

Collaborators get one file they can open by double-clicking. The served version
(rankings.html + data.json) still works and is what to use while iterating.
"""
import json, os
D=os.path.dirname(os.path.abspath(__file__))
html=open(os.path.join(D,"rankings.html")).read()
...
p=os.path.join(D,"rankings_standalone.html")
open(p,"w").write(out)
print(f"wrote rankings_standalone.html ({os.path.getsize(p)/1024:.0f} KB, self-contained)")
